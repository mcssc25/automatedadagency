...
- Lead Intelligence auto-schedule is off by default as of 2026-07-04. Runtime `data/lead-intelligence-settings.json` can enable it from the browser, but Docker/env default is `LEAD_INTELLIGENCE_ENABLED=false` to avoid surprise hourly AI spend.
...
- Lead Intelligence is roster-gated for AI research: it harvests agent emails first, only researches brokerage systems after that brokerage has at least one roster contact and no `Do Not Scrape` office, and marks zero-contact/unharvestable brokerages `Do Not Scrape` after the first zero-contact result so future cycles skip that brokerage instead of spending scrape or AI time.
...
- Lead-intelligence research is OpenRouter-first and must not fall back to Gemini. OpenRouter model routing is constrained to `openrouter/free` or model IDs ending in `:free`; if OpenRouter is unavailable/capped, Lead Intelligence should fail closed instead of spending Gemini budget. Web search is opt-in and may use credits. Keep the auto worker off unless the user explicitly wants it running.
...
- CRM rework commit `4b7b1a2`, brokerage research-signals commit `17dddd4`, response-inbox commit `e4d8392`, real activity log commit `561c0ed`, stale research refresh commit `3124543`, OpenRouter free-model guard commit `7ddf483`, roster-gated research code commit `32f654f`, and OpenRouter-only first-zero-contact suppression commit `0d435f8` were pushed/deployed live on 2026-07-03.
