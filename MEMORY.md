...
- Lead-intelligence research is OpenRouter-first and must not fall back to Gemini. OpenRouter model routing is constrained to `openrouter/free` or model IDs ending in `:free`; if OpenRouter is unavailable/capped, Lead Intelligence should fail closed instead of spending Gemini budget. Web search is opt-in and may use credits.
