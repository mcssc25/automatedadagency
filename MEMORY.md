...
- OpenRouter can be configured from the Integrations UI; saved key lives in ignored runtime `credentials.json`, not browser localStorage or git. Env fallback still works with `OPENROUTER_ENABLED=true` and `OPENROUTER_API_KEY`; never store the key in docs/git.
...
- Nationwide lead intelligence engine runs behind the scenes: hidden DB tables track market cities, brokerage profiles, brokerage offices, roster contacts, and intelligence run logs. Hourly worker is enabled by `LEAD_INTELLIGENCE_ENABLED=true`, uses grounded research to find brokerage/city office roster URLs, then browser-harvests public roster pages into `roster_contacts` instead of cluttering the visible CRM pipeline. Seed plan starts with 20 mid-market cities plus 14 brokerage brand searches per city while search discovery adds smaller/local/regional brokerages.
- Lead-intelligence brokerage discovery, specific office URL lookup, and brokerage tech-stack research can prefer OpenRouter with a daily request cap, ordered free-model rotation, and Gemini fallback. Default first model is `openai/gpt-oss-120b:free`, followed by Gemma 4, Nemotron, Laguna, Qwen, Llama, Hermes, and other free slugs. `OPENROUTER_WEB_SEARCH_ENABLED=true` adds OpenRouter web-search grounding but may use OpenRouter credits even with free text models.
...
- Hardening deployment commit: `c35051f`; async lead scrape commit: `43c3054`; realtor directory commit: `0ae31d0`; brokerage roster commit: `4946727`; realtor roster quality commit: `43cc427`; brokerage-roster-first discovery commit: `649b399`; realtor dedupe/privacy commit: `1d500a7`; CRM auto-approve commit: `2b33db0`; trend keyword commit: `a89a729`; trend preservation commit: `8eb10bd`; lead intelligence base commit: `a91eb02`; lead intelligence reliability commit: `caea108`; OpenRouter lead-intelligence provider commits: `e484757`, docs/deploy status `a1c9dd9`.
- OpenRouter Integrations UI/model rotation commit `f562eee` is deployed live. Production currently reports `configured=false` until the user pastes a key in Integrations or configures `OPENROUTER_API_KEY` in env.
- Lead intelligence production smoke: 20 cities and 280 brand/city offices seeded; KW Birmingham public roster is reachable in a normal browser but production headless Chromium receives a Cloudflare security-verification page, so it is now marked `Blocked` and likely needs a managed/stealth browser provider for protected rosters.
...
- Research & Trends cards should label real numeric views/likes/comments as `Engagement`; qualitative fallback rationale should be labeled `Trend signal`, not `Viral engagement`.
...
- Follow-up fix commit `8eb10bd` (`Preserve trend cards on empty refresh`) is pushed/deployed: production cannot use the Windows `py` last30days plugin path, and a zero-result refresh could overwrite prior trend cards. Research & Trends now skips the unavailable local scraper cleanly, repairs malformed Gemini trend JSON, caches last successful API trend cards, and the UI preserves last good cards when a refresh returns no posts.
- Trend-refresh-preserve deployment backup: `/opt/ad-agency-autopilot/data/backups/deploy-20260702T202056Z-trend-refresh-preserve`; live `/api/trends` smoke returned 6 trend cards, 12 keywords, and 12 searched queries.
