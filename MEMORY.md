...
- OpenRouter can be used for hidden lead-intelligence research by setting `OPENROUTER_ENABLED=true`, `OPENROUTER_API_KEY`, and optionally `LEAD_INTELLIGENCE_RESEARCH_PROVIDER=openrouter`; never store the key in docs/git.
...
- Lead-intelligence brokerage discovery, specific office URL lookup, and brokerage tech-stack research can prefer OpenRouter with a daily request cap and Gemini fallback. `OPENROUTER_WEB_SEARCH_ENABLED=true` adds OpenRouter web-search grounding but may use OpenRouter credits even with free text models.
