...
- Nationwide lead intelligence engine is being added behind the scenes: hidden DB tables track market cities, brokerage profiles, brokerage offices, roster contacts, and intelligence run logs. Hourly worker is controlled by `LEAD_INTELLIGENCE_ENABLED`, uses grounded search to find brokerage/city office roster URLs, then browser-harvests public roster pages into `roster_contacts` instead of cluttering the visible CRM pipeline. Seed plan starts with 20 mid-market cities plus 14 brokerage brand searches per city while search discovery adds smaller/local/regional brokerages.
